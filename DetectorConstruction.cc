...
// -------------------- Build knee SOLIDS and define dependent variables FIRST ----
...
auto kneeWithTibia = new G4UnionSolid("KneeWithTbia", condUnion, tibiaCyl, 0, tibiaRel);
// Fibula
...
// Now that all variables (inner_a, patellaRelPos, z_pig, etc.) are defined, we can build the shell.
...
G4double z_cutoff_local = z_holder_top - z_pig;
G4double sub_size = 50.0*cm;
auto subtractionBox = new G4Box("SubtractionBox", sub_size/2.0, sub_size/2.0, sub_size/2.0);
G4ThreeVector subBoxPos(0.0, 0.0, z_cutoff_local + sub_size/2.0);
...
// Place soft shell in the world. It will be the mother volume for the bones.
new G4PVPlacement(0, G4ThreeVector(x_pig, y_pig, z_pig), softShellLog, "SoftShell", logicCavity, false, 0, false);

// -------------------- Build and Place Knee LOGICAL Volumes -----------------
// Now that the mother volume (softShellLog) exists, create the bone logical volumes.
...
// Place the bones INSIDE the softShellLog, at its local origin (0,0,0).
new G4PVPlacement(0, G4ThreeVector(0, 0, 0), corticalLog, "KneeCortical", softShellLog, false, 0, false);
new G4PVPlacement(0, G4ThreeVector(0, 0, 0), trabLog, "KneeTrabular", softShellLog, false, 0, false);

// --- Patellar tendon and Fascicles (Placed in the world) ---
...
// Place tendon as an independent volume inside the main cavity
new G4PVPlacement(0, tendonWorldPos, tendonLog, "PatellarTendon", logicCavity, false, 0, false);

// -------------------- Place other knee components -------------------------
// Cartilage & menisci approximations (placed in the world cavity)
...
auto makeEllipsePolygon = [&](G4double major, G4double minor, G4int nPts=20) {
    ...
    for (G4int k=0;k<nPts;++k) {
        ...
        double x_local = (minor/2.0) * std::cos(theta);
        double y_local = (major/2.0) * std::sin(theta);
...
for (size_t f=0; f<fiberCenters.size(); ++f) {
    ...
    for (int seg=0; seg<nSegmentsPerFibre; ++seg) {
        ...
        G4double splayAngle = 10.0*deg * std::pow(nd, 1.5);
...
jaw_log->SetVisAttributes(new G4VisAttributes(G4Colour(0.15,0.15,0.15)));
